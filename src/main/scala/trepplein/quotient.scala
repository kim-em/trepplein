...
object quotient {
  // Lean 4 uses capitalized "Quot" names
  val univParams = Vector(Level.Param(Name("u")))
  val A = LocalConst(Binding(Name("α"), Sort(univParams(0)), BinderInfo.Implicit))
  ...
  val B = LocalConst(Binding(Name("β"), Sort(liftUnivParams(1)), BinderInfo.Implicit))
